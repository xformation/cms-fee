 ...
 * 
 */
package com.synectiks.fee.utils;
...
import com.synectiks.fee.utils.ESEvent.EventType;
...
public class SynectiksJPARepo<T, ID extends Serializable>
	...
	public void fireEvent(EventType type, T entity) {
        if (IUtils.isNull(env)) {
        	env = FeeApp.getBean(Environment.class);
        }
        if (IUtils.isNull(rest)) {
        	rest = FeeApp.getBean(RestTemplate.class);
        }
        if (IUtils.isNull(applicationProperties)) {
        	applicationProperties = FeeApp.getBean(ApplicationProperties.class);
        }
